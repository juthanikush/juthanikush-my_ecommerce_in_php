<?php
	...
	function placeShipRocketOrder($con,$token,$order_id){
		...
		$row_order=mysqli_fetch_assoc(mysqli_query($con,"select `order`.*,users.name,users.email,
	users.mobile from `order`,users where `order`.id=$order_id and `order`.user_id=users.id"));
